...
class MCPClient:
    ...
    async def connect_to_server(self, target: str):
        ...
        target may be a URL, a .py/.js script path, or a command line.
        """
        if target.startswith(("http://", "https://")):
            ...
            source = StdioServerParameters(command="node", args=[target], env=None)
        else:
            command, *args = shlex.split(target)
            source = StdioServerParameters(command=command, args=args, env=None)

        self.client = await self.exit_stack.enter_async_context(Client(source))

        response = await self.client.list_tools()
        print(
            f"\nConnected over protocol {self.client.protocol_version} with tools:",
